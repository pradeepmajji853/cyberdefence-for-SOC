    ...
    <div className="min-h-screen bg-bg-primary text-text-primary">
      ...
      <nav className="bg-bg-secondary border-b border-border-primary px-6">
        <div className="flex space-x-6 overflow-x-auto">
          {[
            { id: 'dashboard', label: 'Advanced Dashboard', icon: Activity },
            { id: 'predictor', label: 'AI Threat Prediction', icon: TrendingUp },
      ...
      <main className="p-6">
        {selectedTab === 'dashboard' && (
          <AdvancedDashboard logs={logs} stats={stats} analysis={analysis} />
        )}

        {selectedTab === 'predictor' && (
          <ThreatPredictor logs={logs} />
        )}

        {selectedTab === 'network' && (
          <NetworkMonitor logs={logs} />
        )}

        {selectedTab === 'incidents' && (
          <IncidentResponse logs={logs} analysis={analysis} />
        )}
